...
##   exit_pos     where the exit gate sits; walk into it (with enough lahm) to leave
##   exit_cost    LAHM the gate charges to pass, in lahm (blocks = cost / 50). Rises each level:
##                4/5/6/7/8 blocks. Lahm rots (Player.LAHM_DECAY), so this is a race, not a save-up.
...
## "some chip + one strong" early, more strong later. All tunable here, one place. Enemy density
## also feeds the economy: more (and beefier) enemies = more damage to deal = more lahm to harvest
## against the decay, so difficulty and affordability scale together.
