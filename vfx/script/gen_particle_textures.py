#!/usr/bin/env python3
"""Generate the shared particle textures under vfx/shared/textures/.

Soft additive glow needs a smooth radial-alpha dot (white, so a particle colour
ramp can tint it). These are cross-entity building blocks (used by enemies and
several characters), so they live in the global vfx/shared/textures/ rather than
under any one character. Kept re-runnable so new shapes are easy to add.

    python3 vfx/script/gen_particle_textures.py
"""
...
# Script lives in vfx/script/, so parent.parent is vfx/.
OUT = Path(__file__).resolve().parent.parent / "shared" / "textures"
